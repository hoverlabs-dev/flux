...
def start(port: int = 7722) -> None:
    global _SERVER
    if _SERVER:
        print(f"Flux Blender command server already running on :{port}")
        return
    ...
    thread.start()
    print(f"Flux Blender command server started on :{port}")


def _process_queue():
    while not _QUEUE.empty():
        request = _QUEUE.get()
        namespace = {"__name__": "__flux_blender_exec__"}
        try:
